...
And visit http://localhost:8080/score-ai
...
```bash
$ gcloud functions deploy score-ai --entry-point io.micronaut.gcp.function.http.HttpFunction --runtime java17 --trigger-http
```
...
```bash
$ YOUR_HTTP_TRIGGER_URL=$(gcloud functions describe score-ai --format='value(httpsTrigger.url)')
```

You can then use this variable to test the function invocation:

```bash
$ curl -i $YOUR_HTTP_TRIGGER_URL/score-ai
```
